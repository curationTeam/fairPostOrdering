...
class Simulation:
    ...
    '''
    honest and selfish content creators. users only vote and do not create content (and are honest)
    profile = ["honest", "selfish", "user"]
    ...
    def __init__(self, spvec, rounds, noProfiles, a, b, regen, att_span, lik_mat, seed):
        self.profile = ("honest", "selfish", "user")
    ...
    def create_posts(self, likability_matrix):
        assert len(likability_matrix) == self.noProfiles[0] + bool(self.noProfiles[1])
        for likability in likability_matrix:
          assert len(likability) == sum(self.noProfiles)

        # last post is generated by the first selfish player
    ...
    def init_players(self, noPosts):
        ...
        for noProfile in self.noProfiles:
            ring_leader_id = None
            if profile_index == 1: # selfish strategy
                ring_leader_id = self.noProfiles[0] # id of the first selfish player
            ...
            for _i in range(0, noProfile):
                ...
                self.rounds, noPosts, ring_leader_id))
                index += 1
            profile_index += 1
        return players
