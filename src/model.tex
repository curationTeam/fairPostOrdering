\section{Model}
  ...
  \subsection{Properties of Post Voting Systems}
    ...
    \begin{definition}[Ideal Post Order]
      Let $\mathcal{P}$ a list of posts. We say that $\mathcal{P}$ is in
      \emph{ideal order} and that the property
      $\textsc{Ideal}\left(\mathcal{P}\right)$ holds if
      \begin{equation*}
        \forall i < j \in \left|\mathcal{P}\right|,
        \mathrm{sc}\left(\mathcal{P}\left[i\right]\right) \geq
        \mathrm{sc}\left(\mathcal{P}\left[j\right]\right) \enspace.
      \end{equation*}
    \end{definition}
    ...
    \paragraph{Discussion}
      ...
      I suggest that we turn to a simulation-based approach. The Environment
